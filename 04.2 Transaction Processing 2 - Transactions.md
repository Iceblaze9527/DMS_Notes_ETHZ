...
one could argue that there is no need to recover anything as `x` has been rewritten, but if `T2` aborts, then `T2` cannot be recovered by restoring a before image
